...
export class HistoColumnTargetComponent extends HistoColumnComponent {
  ...
  private getTargetValue(d: number): string {
    return formatNumberToString(DEH.get(this.data.targetLevel['name'])[this.data.targetLevel['ids'][d]][DEH.get(this.data.targetLevel['structure']).indexOf(this.data.targetLevel['volumeIdentifier'])]);
  }
