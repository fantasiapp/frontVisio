...
export class CD{ //For ComputeDescription
  ...
  private static getObjectifDrv(node:any, dn=false){
    ...
    if (node.nature == 'drv') targetDrv = node.children.map((agentNode:any) => DEH.getTarget('agent', agentNode.id, dn)).reduce((acc:number, value:number) => acc + value, 0);
  ...
  static computeDescriptionWidget(slice:any): [number, number, number][]{
    ...
    let objectiveWidget: [number, number, number] = [
      (relevantNode.children as any[]).map(subLevelNode => DEH.getTarget(subLevelNode.nature, subLevelNode.id)).reduce((acc, value) => acc + value, 0),
      (relevantNode.children as any[]).map(subLevelNode => DEH.getTarget(subLevelNode.nature, subLevelNode.id, true)).reduce((acc, value) => acc + value, 0),
      0],
